...
class ReplayBuffer:
    ...
    def learn(self):
        ...
        reward_batch = tf.cast(tf.convert_to_tensor(self.reward_buffer[batch_indices]), tf.float32)
        ...
        target_q1 = target_critic1([next_state_batch, next_actions])
        target_q2 = target_critic2([next_state_batch, next_actions])
        y = reward_batch + GAMMA * tf.minimum(target_q1, target_q2)

        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
            q1 = critic_model1([state_batch, action_batch])
            q2 = critic_model2([state_batch, action_batch])
            critic_loss1 = tf.reduce_mean((y - q1) ** 2)
            critic_loss2 = tf.reduce_mean((y - q2) ** 2)

        grads1 = tape1.gradient(critic_loss1, critic_model1.trainable_variables)
        grads2 = tape2.gradient(critic_loss2, critic_model2.trainable_variables)
        ...
        if self.update_step % self.policy_delay == 0:
            with tf.GradientTape() as tape_a:
                ...
                q_val = critic_model1([state_batch, actions])
                actor_loss = -tf.reduce_mean(q_val)

            actor_grads = tape_a.gradient(actor_loss, actor_model.trainable_variables)
