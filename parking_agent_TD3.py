...
# ---------------------------------------------------------------------------------------------------
# Twin Delayed Deep Deterministic (TD3) AGENT CLASS
# ---------------------------------------------------------------------------------------------------
class TD3Agent:
    def __init__(self, policy_delay=2, target_noise_std=0.2, target_noise_clip=0.5):
        # TD3 parameter
    ...
    def get_critic(self, index, model_name='', terminated=False, show_summary=False):
        ...
        if TRAINING_INDICATOR == 1 and os.path.exists(model_weights_file_name):
            ...
            model.set_weights(globals()[f'critic_model{index}'].get_weights())
...
class ReplayBuffer:
    ...
    def update(self, state_batch, action_batch, reward_batch, next_state_batch):
        ...
        noise = tf.clip_by_value(
            tf.random.normal(shape=next_actions.shape, stddev=self.target_noise_std),
            -self.target_noise_clip, self.target_noise_clip
        )
        next_actions = tf.clip_by_value(next_actions + noise, -1.0, 1.0)

        #target Q
        target_q1 = target_critic1([next_state_batch, next_actions], training=True)
        target_q2 = target_critic2([next_state_batch, next_actions], training=True)
        y = reward_batch + GAMMA * tf.minimum(target_q1, target_q2)

        #2 critic update
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
            q1 = critic_model1([state_batch, action_batch], training=True)
            q2 = critic_model2([state_batch, action_batch], training=True)
            loss1 = tf.reduce_mean((y - q1)**2)
            loss2 = tf.reduce_mean((y - q2)**2)

        grads1 = tape1.gradient(loss1, critic_model1.trainable_variables)
        grads2 = tape2.gradient(loss2, critic_model2.trainable_variables)
        critic_optimizer1.apply_gradients(zip(grads1, critic_model1.trainable_variables))
        critic_optimizer2.apply_gradients(zip(grads2, critic_model2.trainable_variables))
        ...
        if self.update_step % self.policy_delay == 0:
            ...
            with tf.GradientTape() as tape_a:
                ...
                q_val = critic_model1([state_batch, actions], training=True)
                actor_loss = -tf.reduce_mean(q_val)
            grads_a = tape_a.gradient(actor_loss, actor_model.trainable_variables)
            actor_optimizer.apply_gradients(zip(grads_a, actor_model.trainable_variables))

            # target network soft update
            update_target(target_actor.variables, actor_model.variables)
            update_target(target_critic1.variables, critic_model1.variables)
            update_target(target_critic2.variables, critic_model2.variables)
...
if __name__ == '__main__':
    ...
    if TRAINING_INDICATOR == 0:
        ...
        try:
            ...
            critic_model1 = agent.get_critic(1, show_summary=True)
            critic_model2 = agent.get_critic(2, show_summary=True)
